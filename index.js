import{a as k,i as g,S as ve}from"./assets/vendor-Cq1ZEsD1.js";(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const r of document.querySelectorAll('link[rel="modulepreload"]'))s(r);new MutationObserver(r=>{for(const a of r)if(a.type==="childList")for(const i of a.addedNodes)i.tagName==="LINK"&&i.rel==="modulepreload"&&s(i)}).observe(document,{childList:!0,subtree:!0});function n(r){const a={};return r.integrity&&(a.integrity=r.integrity),r.referrerPolicy&&(a.referrerPolicy=r.referrerPolicy),r.crossOrigin==="use-credentials"?a.credentials="include":r.crossOrigin==="anonymous"?a.credentials="omit":a.credentials="same-origin",a}function s(r){if(r.ep)return;r.ep=!0;const a=n(r);fetch(r.href,a)}})();const o={burgerBtnOpenMenu:document.querySelector(".icon-burger"),burgerBtnCloseMenu:document.querySelector(".icon-close"),headerMenuContainer:document.querySelector(".header-menu-container"),loaderOverlay:document.querySelector(".loader-in"),modalForm:document.querySelector(".feedback-mod"),stars:document.querySelectorAll(".star-rating path"),nameInput:document.querySelector('.feedback-mod [name="name"]'),messageInput:document.querySelector('.feedback-mod [name="message"]'),modalOverlayArtists:document.querySelector(".modal-overlay-artists"),closeBtn:document.querySelector(".close-btn-mod"),feedbackBtn:document.querySelector(".feedback-btn"),menuAnchorAbout:document.querySelector("#menu-anchor-about"),menuAnchorArtists:document.querySelector("#menu-anchor-artists"),menuAnchorReviews:document.querySelector("#menu-anchor-reviews"),artistsGrid:document.getElementById("artists-card-id"),loadMoreBtn:document.getElementById("load-more-btn"),loader:document.querySelector(".modal-loader"),modalOverlay:document.querySelector(".modal-overlay"),body:document.querySelector("body"),closeModalBtn:document.querySelector(".close-btn"),titleName:document.querySelector(".modal-info-title"),aboutArtist:document.querySelector(".modal-info-card"),modalAlboms:document.querySelector(".modal-alboms"),artistsSection:document.querySelector("#artists-card-id")},ge=8,A="https://sound-wave.b.goit.study/api",Q="/artists",he="/genres",ye="/albums",X="/feedbacks";async function be(e=1,t="",n="",s=""){const r={limit:ge,page:e,...t&&{name:t},...n&&{sortName:n},...s&&{genre:s}};try{return(await k.get(`${A}${Q}`,{params:r})).data}catch{g.error({title:"Error",message:"Failed to fetch artists.",position:"topRight"})}}async function Le(){try{return(await k.get(`${A}${he}`)).data}catch{g.error({title:"Error",message:"Failed to fetch genres.",position:"topRight"})}}async function we(e){try{return(await k.get(`${A}${Q}/${e}${ye}`)).data}catch{g.error({title:"Error",message:"Failed to fetch albums.",position:"topRight"})}}async function Ee(e=1){try{return(await k.get(`${A}${X}`,{params:{page:e}})).data}catch{g.error({title:"Error",message:"Failed to fetch feedbacks.",position:"topRight"})}}async function ke(e){try{const t=await k.post(`${A}${X}`,e);return g.success({title:"Success",message:"Feedback submitted successfully.",position:"topRight",timeout:3e3}),t.data}catch{g.error({title:"Error",message:"Failed to submit feedback.",position:"topRight"})}}const{loaderOverlay:Z,modalForm:ee,stars:te,nameInput:j,messageInput:Y,modalOverlay:T,closeBtn:Ae,feedbackBtn:Se}=o;function $e(){Z.classList.remove("hidden")}function Me(){Z.classList.add("hidden")}let f=0;te.forEach((e,t)=>{e.addEventListener("mouseenter",()=>{B(t+1)}),e.addEventListener("mouseleave",()=>{B(f)}),e.addEventListener("click",()=>{f=t+1,B(f)})});function B(e){te.forEach((t,n)=>{n<e?t.setAttribute("fill","#a76cdb"):t.setAttribute("fill","#ffffff")})}function Be(){T.classList.add("is-open"),document.addEventListener("keydown",se)}Se.addEventListener("click",Be);function q(){T.classList.remove("is-open"),document.removeEventListener("keydown",se)}Ae.addEventListener("click",q);T.addEventListener("click",e=>{e.target===T&&q()});function se(e){e.key==="Escape"&&q()}function Ne(){let e=!0,t=[];return(j.value.trim().length<2||j.value.trim().length>16)&&(e=!1,t.push("Name must be between 2 and 16 characters")),(Y.value.trim().length<10||Y.value.trim().length>512)&&(e=!1,t.push("Message must be between 10 and 512 characters.")),(f<1||f>5)&&(e=!1,t.push("Rating must be between 1 and 5 stars.")),e||t.forEach(n=>{g.error({title:"Error",message:n,position:"topRight",timeout:3e3,pauseOnHover:!0})}),e}function Te(){ee.reset(),B(0),f=0}ee.addEventListener("submit",async e=>{if(e.preventDefault(),!Ne())return;const t={name:j.value.trim(),rating:f,descr:Y.value.trim()};$e(),await ke(t),q(),Te(),Me()});const c="/goit-js-project-work-team-1/assets/sprite-wQKTNHgQ.svg";function qe(e,t=c){const n=Math.floor(e),s=e-n,r=Math.round(s*100);let a="";for(let i=1;i<=5;i++)i<=n?a+=`
          <div class="star">
      ...
      <li class="star-svg">
      ...
      </li>`}function Ie(e,t){const n=e.map(s=>{var l;const r=s.strArtistThumb||"https://via.placeholder.com/350x350?text=No+Image",a=s.genres||[],i=((l=s.genres)==null?void 0:l.map(y=>`<li class="genre-tag">${y}</li>`).join(""))||"",h=s.strBiographyEN||"Короткий опис для цього артиста відсутній.";return`
        <li class="artist-card">
          ...
          <div class="artist-card-content">
            ...
            <p class="artist-card-description">${h}</p>
  ...
  `}function U(e,t,n){const s=document.querySelector(".swiper-button-prev"),r=document.querySelector(".swiper-button-next");s.classList.toggle("disabled",e===t),r.classList.toggle("disabled",e===n)}async function He(){const{data:e}=await Ee(),t=e.slice(0,30),n=0,s=t.length-1,r=Math.floor(t.length/2);document.querySelector(".swiper-wrapper").innerHTML=t.map(De).join("");const a=new ve(".swiper",{slidesPerView:1,loop:!1,navigation:{nextEl:".swiper-button-next",prevEl:".swiper-button-prev"}}),i=document.querySelector(".swiper-pagination");i.innerHTML=`
    ...
    <span class="swiper-pagination-bullet" data-i="${s}"></span>`,i.addEventListener("click",l=>{l.target.classList.contains("swiper-pagination-bullet")&&a.slideTo(+l.target.dataset.i)});function h(){const l=a.realIndex;document.querySelectorAll(".swiper-pagination-bullet").forEach(y=>{y.classList.toggle("active",+y.dataset.i===l)}),U(l,n,s)}a.on("slideChange",()=>{h(),U(a.realIndex,n,s)}),h()}He();let d=1,L="",w="",S="";const Fe=8,E=document.getElementById("searchInput"),m=document.getElementById("genreDropdown"),I=document.getElementById("genreToggle"),re=document.getElementById("genreList"),p=document.getElementById("sortingDropdown"),O=document.getElementById("sortingToggle"),xe=document.getElementById("sortingList"),Re=document.getElementById("resetBtn"),Pe=document.getElementById("resetFiltersBtn"),V=document.querySelector(".icon-chevron"),_e=document.getElementById("artists-card-id"),G=document.querySelector(".spinner"),W=o.artistsGrid,N=o.loadMoreBtn,J=document.getElementById("emptyState"),Ge=document.querySelector(".search-and-filters-opener"),z=document.querySelector(".filters-panel"),b=document.querySelector(".scrollBtnUp");function $(){m.classList.remove("open"),p.classList.remove("open")}I.addEventListener("click",()=>{m.classList.contains("open")?(m.classList.remove("open"),m.querySelector(".icon-chevron").innerHTML=`<use href="${c}#icon-chevron-up-arrow"></use>`):($(),m.classList.add("open"),m.querySelector(".icon-chevron").innerHTML=`<use href="${c}#icon-chevron-down-arrow"></use>`)});O.addEventListener("click",()=>{p.classList.contains("open")?(p.classList.remove("open"),p.querySelector(".icon-chevron").innerHTML=`<use href="${c}#icon-chevron-up-arrow"></use>`):($(),p.classList.add("open"),p.querySelector(".icon-chevron").innerHTML=`<use href="${c}#icon-chevron-down-arrow"></use>`)});document.addEventListener("click",e=>{!m.contains(e.target)&&!p.contains(e.target)&&$()});Ge.addEventListener("click",()=>{z.classList.toggle("open"),z.classList.contains("open")?V.innerHTML=`<use href="${c}#icon-chevron-down-arrow"></use>`:V.innerHTML=`<use href="${c}#icon-chevron-up-arrow"></use>`});async function je(){const e=await Le();e!=null&&e.length&&(re.innerHTML='<div class="dropdown-item" data-value="">Default</div>'+e.map(t=>`<div class="dropdown-item" data-value="${t.genre}">${t.genre}</div>`).join(""))}je();xe.addEventListener("click",e=>{e.target.classList.contains("dropdown-item")&&(w=e.target.dataset.value==="default"?"":e.target.dataset.value,O.querySelector(".dropdown-title").textContent=`${w||"Default"}`,d=1,$(),u(!0))});re.addEventListener("click",e=>{e.target.classList.contains("dropdown-item")&&(L=e.target.dataset.value==="Default"?"":e.target.dataset.value,I.querySelector(".dropdown-title").textContent=`${L||"Default"}`,d=1,$(),u(!0))});E.addEventListener("keydown",e=>{e.key==="Enter"&&(S=E.value.trim(),d=1,u(!0))});z.addEventListener("submit",e=>{e.preventDefault(),S=E.value.trim(),d=1,u(!0)});Re.addEventListener("click",()=>{L="",w="",S="",E.value="",I.textContent="Default",O.textContent="Default",d=1,u(!0)});Pe.addEventListener("click",()=>{L="",w="",S="",E.value="",I.textContent="Default",O.textContent="Default",d=1,u(!0)});N.addEventListener("click",()=>{d++,u()});async function u(e=!1){N.style.display="none",J.classList.add("hidden"),G.classList.remove("hidden"),e&&(W.innerHTML="");const t={page:d,name:S,sortName:w||void 0,genre:L||void 0},n=await be(t.page,t.name,t.sortName,t.genre),s=(n==null?void 0:n.artists)||[];if(e&&s.length===0){J.classList.remove("hidden"),_e.innerHTML="",G.classList.add("hidden");return}Ie(s,W),s.length===Fe?N.style.display="flex":N.style.display="none",G.classList.add("hidden")}u(!0);function Ye(){window.scrollY>(document.body.scrollHeight-window.innerHeight)/2?b.classList.add("visible-scrollBtn"):b.classList.remove("visible-scrollBtn");function e(){window.scrollTo({top:0,behavior:"smooth"})}b.classList.contains("visible-scrollBtn")?b.addEventListener("click",e):b.removeEventListener("click",e)}document.addEventListener("scroll",Ye);function oe(e){e.target===o.modalOverlayArtists&&C()}function ae(e){o.modalOverlayArtists.classList.contains("is-open")&&e.key==="Escape"&&C()}function ze(e,t){const n=parseInt(e,10),s=parseInt(t,10);return isNaN(n)&&isNaN(s)?"information missing":!isNaN(n)&&isNaN(s)?`${n}–present`:!isNaN(n)&&!isNaN(s)?`${n}–${s}`:"information missing"}function Ke(e){if(typeof e=="string"){const a=parseInt(e,10);if(isNaN(a))return"N/A";e=a}if(e==null||isNaN(e))return"N/A";const t=Math.floor(e/1e3),n=Math.floor(t/60),s=t%60,r=s<10?`0${s}`:s;return`${n}:${r}`}async function Ue(e,t=[]){Ce();const n=o.aboutArtist.querySelector(".modal-info-description");n&&(n.innerHTML=""),o.modalAlboms.innerHTML="";const s=await we(e),{strArtist:r,strArtistThumb:a,strGender:i,intMembers:h,strCountry:l,strBiographyEN:y,intFormedYear:ie,intDisbandedYear:ce,albumsList:D}=s;o.titleName.textContent=r||"N/A";const le=ze(ie,ce);let H="";t&&Array.isArray(t)&&t.length>0&&(H=t.map(M=>`<span class="genre-tag">${M.trim()}</span>`).join(""));const de=` <img class="ph-artist" src="${a}" alt="${r}" />
  <div class="modal-info-description">
            <div class="modal-info-list">
                ...
                <div class="wrapp">
                    ...
                    <div class="modal-info-item">
                        ...
                        <span class="detail-value">${h||"N/A"}</span>
                    </div>
                    <div class="modal-info-item">
            ...
            <div class="biography">
                ...
                <p class="modal-info-bio">
                    ${y||"No biography available."}
                </p>
            </div>
            ${H?`<div class="genres">${H}</div>`:""}
            </div>`;o.aboutArtist.innerHTML=de;let F="";D&&D.length>0?D.forEach(M=>{const ue=M.strAlbum||"Unknown Album",x=M.tracks;let R="";x&&x.length>0?x.forEach((P,me)=>{const pe=(me+1)%2===0?"albom-track even":"albom-track odd",_=P.movie,fe=typeof _=="string"&&_.trim()!==""?`<a class="link-icon-youtube" href="${_}" target="_blank" rel="noopener noreferrer" aria-label="Watch on YouTube">
